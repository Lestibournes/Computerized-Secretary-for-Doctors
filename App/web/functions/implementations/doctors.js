...
async function getClinics(user_id, city) {
	...
	.then(doctor_snaps => {
		...
		// clinic_promises.push(
		// 	db.collection("clinics").where("owner", "==", user_id)
		// 	.then(clinic_snaps => {
		// 		const clinics = [];

		// 		for (const clinic_snap of clinic_snaps) {
		// 			if (clinic_snap.exists && (!city || stringContains(clinic_snap.id, city))) {
		// 				const clinic_data = clinic_snap.data();
		// 				clinic_data.id = clinic_snap.id;
		// 				clinics.push(clinic_data);
		// 			}
		// 		}

...
describe("Creating shifts", () => {
	test("Create shift in the current user's clinic", async () => {
		...
		const thisShiftsRef = clinicRef.collection("doctors").doc(thisAuth.uid).collection("shifts");
		...
		const otherShiftsRef = clinicRef.collection("doctors").doc(otherAuth.uid).collection("shifts");
	
		// Performing the test:
		for (const shiftData of thisShiftData) {
			expect(await firebase.assertSucceeds(thisShiftsRef.add(shiftData)));
			expect(await firebase.assertSucceeds(otherShiftsRef.add(shiftData)));
		}
	...
	test("Create a shift for another doctor in another user's clinic", async () => {
		...
		const ShiftsRef = getFirestore(thisAuth).collection("clinics").doc(clinicID).collection("doctors").doc(otherAuth.uid).collection("shifts");
	
		// Performing the test:
		for (const shiftData of thisShiftData) {
			expect(await firebase.assertFails(ShiftsRef.add(shiftData)));
		}
	...
	test("Create a shift for the current user in another user's clinic", async () => {
		...
		const thisShiftsRef = getFirestore(thisAuth).collection("clinics").doc(clinicID).collection("doctors").doc(thisAuth.uid).collection("shifts");
